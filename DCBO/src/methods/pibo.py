...
from src.bases.pibo_root import Root
from src.bayes_opt.intervention_computations import evaluate_acquisition_function_with_prior
...
class PIBO(Root):
    ...
    def run(self, priors_regret):
        ...
        for temporal_index in trange(self.T, desc="Time index"):
            ...
            for it in range(self.number_of_trials):
                ...
                self._per_trial_computations(temporal_index, it, target, assigned_blanket, priors_regret)
    ...
    def _evaluate_acquisition_functions(self, temporal_index, current_best_global_target, it, priors_regret):

        # print('sono in _evaluate_acquisition_functions e mi arrivano i prior')
        ...
        for es in self.exploration_sets:
            ...
            (self.y_acquired[es], self.corresponding_x[es],) = evaluate_acquisition_function_with_prior(
