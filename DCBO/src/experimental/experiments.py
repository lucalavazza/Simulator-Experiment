...
def run_all_opt_models(
    ...
    for method in methods_list:
        ...
        assert method in ["ABO", "DCBO", "BO", "CBO", "PIBO", "DCPIBO"], (
        ...
        if method in ["DCBO", "CBO", "DCPIBO"]:
            # Args common to DCBO and CBO and DCPIBO
        ...
        if method in ["BO", "ABO", "CBO", "DCBO"]:
            ...
            if method == "DCPIBO":
                ...
                alg_input_params["transfer_hp_o"] = transfer_hp_o
                alg_input_params["transfer_hp_i"] = transfer_hp_i
                # print("For the time being I'm useless, but at least I exist.")

            else:
                algorithm = PIBO
                alg_input_params["number_of_trials"] = number_of_trials_BO_ABO
                print(number_of_trials_BO_ABO)
                # print("For the time being I'm also useless, but at least I exist as well.")

            print("\n\t>>>" + method + "\n")
            model = algorithm(**alg_input_params)
            model.run(priors_regret)
            models_list.append(model)
